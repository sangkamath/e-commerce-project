        ...
        if(params) {
            Object.keys(params).forEach((key) => {
                if (params[key as keyof ReviewQueryParams] !== undefined) {
                    url.searchParams.append(key, params[key as keyof ReviewQueryParams]!.toString());
                }
            });
        }
...
export const subscribeEmail = async (email: string) => {
    try {
        const url = "https://www.greatfrontend.com/api/projects/challenges/e-commerce/subscribe";
